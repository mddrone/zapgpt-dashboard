...
export function Header() {
  ...
  const title = PAGE_TITLES[pathname] ?? 'MD Drone'
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between h-14 px-4 md:px-6 bg-black/90 backdrop-blur-sm border-b border-zinc-900/80">
      {/* Left: page title (push right on mobile to avoid hamburger) */}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-white text-xs font-bold shadow-md">
          MD
        </div>
      </div>
    </header>
