...
// ── Cores ────────────────────────────────────────────────────────────────────
const CHART_COLORS = ['#22c55e','#3b82f6','#a855f7','#eab308','#06b6d4','#ec4899','#f97316','#10b981','#f43f5e','#8b5cf6','#14b8a6','#fb923c']
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              type="text" placeholder="Ex: Almoço, Uber, Sinal cliente..."
          ...
          {/* Referência */}
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">Referência (cliente/evento)</label>
            <input
              type="text" placeholder="Ex: Casamento Silva, Debutante Maria..."
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Controle de entradas e saídas</p>
        </div>
                ...
                {periodoOptions.map(p => (
                  <button
                    ...
                    className={cn('w-full text-left px-4 py-2 text-sm transition-colors',
                      periodo === p.value ? 'text-green-400 bg-green-500/10' : 'text-zinc-300 hover:text-white hover:bg-zinc-800')}
                  >
                    {p.label}
                  </button>
                ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={cn('card p-5 flex flex-col gap-3 col-span-2 lg:col-span-1',
          metrics.saldo >= 0 ? 'border-green-500/25' : 'border-red-500/25')}>
          <div className="flex items-start justify-between">
            <div className={cn('p-2.5 rounded-xl', metrics.saldo >= 0 ? 'bg-green-500/15' : 'bg-red-500/15')}>
              <Wallet size={20} className={metrics.saldo >= 0 ? 'text-green-400' : 'text-red-400'} />
            </div>
            <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full',
              metrics.saldo >= 0 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400')}>
              {metrics.saldo >= 0 ? '▲ positivo' : '▼ negativo'}
            </span>
          </div>
          <div>
            <p className={cn('text-2xl font-bold tabular-nums', metrics.saldo >= 0 ? 'text-green-400' : 'text-red-400')}>
        ...
        {/* Entradas */}
        <div className="card p-5 flex flex-col gap-3 border-green-500/20">
          <div className="flex items-start justify-between">
          ...
          <div>
            <p className="text-2xl font-bold text-green-400 tabular-nums">{formatCurrency(metrics.totalEntradas)}</p>
                    ...
                    <tr key={t.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors group">
                      ...
                      <td className="px-5 py-3.5">
                        <span className={cn('badge',
                          isEntrada ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400')}>
